...
Drives .github/workflows/cross-platform-perf.yaml across ubuntu / macOS /
windows runners. Each subcommand maps to one workflow step; the workflow
shells out to this script so per-platform logic stays in one Python file
instead of being duplicated across yaml `run:` blocks.

Subcommands:
- download-dataset    download the qdrant_random_ints_1m benchmark dataset
- run-benchmark       run raw_hybrid_collection_benchmark.py on one platform
- append-summary      append one platform's result to GITHUB_STEP_SUMMARY
- aggregate-summary   merge all platforms' results into a comparison table

When to run:
- Driven by the cross-platform-perf workflow (consumes GITHUB_STEP_SUMMARY)
- Manually for local debugging: `uv run python <script> <subcommand>`

Usage (workflow steps):
    uv run python scripts/ci/cross_platform_perf.py download-dataset
    uv run python scripts/ci/cross_platform_perf.py run-benchmark
    uv run python scripts/ci/cross_platform_perf.py append-summary
    uv run --no-project python scripts/ci/cross_platform_perf.py aggregate-summary

Configuration:
    Reads BENCHMARK_LABEL / BENCHMARK_QUERY_NUM / BENCHMARK_TOPK / ... from env.
    Every env var has a CLI-flag equivalent (--label, --query-num, --topk, ...).
"""
