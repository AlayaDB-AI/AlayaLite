...
/**
 * SQ4 IP SIMD Distance Benchmark
 *
 * Usage: ./ip_sq4_benchmark [dim1 dim2 ...]
...
void pack_sq4(std::vector<uint8_t>& packed, size_t dim, unsigned seed) {
  ...
  for (size_t i = 0; i < num_bytes; ++i) {
    ...
    auto hi = static_cast<uint8_t>(dist(rng));
    packed[i] = (hi << 4) | lo;
  }
}
...
auto run_benchmarks_for_dim(size_t dim) -> DimResults {
  ...
  results.generic_.ns_per_call_ =
      run_benchmark(alaya::simd::ip_sqr_sq4_generic, x.data(), y.data(),
  ...
  if (features.avx2_ && features.fma_) {
    ...
    results.avx2_.ns_per_call_ =
        run_benchmark(alaya::simd::ip_sqr_sq4_avx2, x.data(), y.data(),
                      min_vals.data(), max_vals.data(), dim, kBenchmarkIterations);
  ...
  if (features.avx512f_) {
    ...
    results.avx512_.ns_per_call_ =
        run_benchmark(alaya::simd::ip_sqr_sq4_avx512, x.data(), y.data(),
                      min_vals.data(), max_vals.data(), dim, kBenchmarkIterations);
    results.avx512_.speedup_ = baseline_ns / results.avx512_.ns_per_call_;
  }
#endif

  // Best (get_ip_sqr_sq4_func with auto dispatch)
  results.best_.ns_per_call_ =
      run_benchmark(alaya::simd::get_ip_sqr_sq4_func(), x.data(), y.data(),
                    min_vals.data(), max_vals.data(), dim, kBenchmarkIterations);
...
void print_comparison_table(const std::vector<DimResults>& all_results) {
  std::cout << "\n## SQ4 IP SIMD Distance Performance Comparison\n\n";
  ...
  for (const auto& r : all_results) {
    ...
    // Best (get_ip_sqr_sq4_func with dispatch)
  ...
  std::cout << "- **AUTO** = get_ip_sqr_sq4_func() with auto dispatch\n";
...
auto main(int argc, char* argv[]) -> int {
  std::cout << "# SQ4 IP SIMD Distance Benchmark\n\n";
