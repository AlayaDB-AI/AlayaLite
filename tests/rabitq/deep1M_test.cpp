...
//   "; auto f_rescale_ptr = space->get_f_rescale_ptr(centroid); for (size_t i = degree_bound -
//   check_precomputed_data_cnt; i < degree_bound; ++i) {
...
//   search_job->rabitq_search_solo(queries_.data() + (query_index * query_dim_), topk,
//   results.data(),
//                                  ef);
  ...
  for (size_t r = 0; r < test_round; ++r) {
    for (size_t i = 0; i < efs.size(); ++i) {  // NOLINT
      ...
      for (uint32_t n = 0; n < query_num_; ++n) {
        timer.reset();
        search_job->rabitq_search_solo(queries_.data() + (n * query_dim_), topk,
                                            results.data(), ef);
