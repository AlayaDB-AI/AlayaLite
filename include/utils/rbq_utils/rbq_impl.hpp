...
#include "./fast_scan/fastscan.hpp"
#include "defines.hpp"
#include "pack_excode.hpp"
#include "space.hpp"

// NOLINTBEGIN
namespace alaya::rabitq_impl {
 ...
 */
template <typename T>
inline RowMajorArray<T> one_bit_code(const T *data, const T *centroid, size_t dim,
                                     int *binary_code) {
 ...
 */
template <typename T>
inline void one_bit_code_with_factor(const T *data, const T *centroid, size_t dim, int *binary_code,
                                     T &f_add, T &f_rescale, T &f_error,
...
inline void one_bit_compact_code(const T *data, const T *centroid, size_t padded_dim,
                                 TC *compact_code, T &f_add, T &f_recale, T &f_error,
                                 MetricType metric_type = MetricType::L2) {
  // binary code
  std::vector<int> binary_code(padded_dim);

  // get binary code
  one_bit_code_with_factor(data, centroid, padded_dim, binary_code.data(), f_add, f_recale, f_error,
                           metric_type);
...
inline void one_bit_compact_codes(const T *data, const T *centroid, size_t num, size_t padded_dim,
                                  TC *compact_code, T *f_add, T *f_rescale, T *f_error,
                                  MetricType metric_type = MetricType::L2) {
  constexpr size_t kTypeBits = sizeof(TC) * 8;

#pragma omp parallel for if (Parallel)
  for (size_t i = 0; i < num; ++i) {
    one_bit_compact_code(data + (padded_dim * i), centroid, padded_dim,
                         compact_code + (padded_dim / kTypeBits * i), f_add[i], f_rescale[i],
                         f_error[i], metric_type);
  }
}

// ! padded_dim % 64 == 0
template <typename T, bool Parallel = false>
inline void one_bit_batch_code(const T *data, const T *centroid, size_t num, size_t padded_dim,
                               uint8_t *packed_code, T *f_add, T *f_recale, T *f_error,
                               MetricType metric_type = MetricType::L2) {
  std::vector<uint8_t> compact_codes(num * padded_dim / 8);

  one_bit_compact_codes<T, uint8_t, Parallel>(data, centroid, num, padded_dim, compact_codes.data(),
                                              f_add, f_recale, f_error, metric_type);
...
template <typename T, typename TP>
inline T ex_bits_code(const T *residual, size_t dim, size_t ex_bits, TP *ex_code,
                      double t_const = -1) {
...
template <typename T, typename TP>
inline void ex_bits_code_with_factor(const T *data, const T *centroid, size_t dim, size_t ex_bits,
                                     TP *ex_code, T &f_add_ex, T &f_rescale_ex, T &f_error_ex,
                                     MetricType metric_type = MetricType::L2, double t_const = -1) {
  ConstRowMajorArrayMap<T> data_arr(data, 1, dim);
  ConstRowMajorArrayMap<T> cent_arr(centroid, 1, dim);
  ...
  // get factors
  RowMajorArray<int> total_code = RowMajorArrayMap<TP>(ex_code, 1, dim).template cast<int>();
  for (size_t i = 0; i < dim; ++i) {
...
template <typename T, typename TP>
inline void ex_bits_compact_code(const T *data, const T *centroid, size_t padded_dim,
                                 size_t ex_bits, TP *compact_code, T &f_add_ex, T &f_rescale_ex,
                                 T &f_error_ex, MetricType metric_type = MetricType::L2,
                                 double t_const = -1) {
  std::vector<uint8_t> ex_code(padded_dim);

  ex_bits_code_with_factor(data, centroid, padded_dim, ex_bits, ex_code.data(), f_add_ex,
                           f_rescale_ex, f_error_ex, metric_type, t_const);
...
static inline void rabitq_scalar_impl(
    const T *data, const T *centroid, size_t dim, size_t total_bits, TP *total_code, T &delta,
    T &vl, double t_const = -1,
  ...
  if (ex_bits > 0) {
    ex_bits::ex_bits_code<T, TP>(residual_arr.data(), dim, ex_bits, total_code, t_const);
  }
...
template <typename T, typename TP>
static inline void rabitq_full_impl(const T *data, const T *centroid, size_t dim, size_t total_bits,
                                    TP *total_code, T &f_add, T &f_rescale, T &f_error,
                                    MetricType metric_type = MetricType::L2, double t_const = -1) {
  std::vector<int> binary_code(dim);
  size_t ex_bits = total_bits - 1;
  ...
  if (ex_bits > 0) {
    ex_bits::ex_bits_code_with_factor<T, TP>(data, centroid, dim, ex_bits, total_code, f_add,
                                             f_rescale, f_error, metric_type, t_const);
  }
...
struct RabitqConfig {
  double t_const = -1;  // NOLINT
  explicit RabitqConfig() = default;
  RabitqConfig(RabitqConfig const &) = default;
  RabitqConfig(RabitqConfig &&) = default;
  auto operator=(const RabitqConfig &) -> RabitqConfig & = default;
  auto operator=(RabitqConfig &&) -> RabitqConfig & = default;
};

// init config for faster quantization rabitq, for total_bits > 0, store const rescaling
// factor in RabitqConfig
inline auto faster_config(size_t dim, size_t total_bits) -> RabitqConfig {
  RabitqConfig config;
  if (total_bits > 1) {
    config.t_const = ex_bits::get_const_scaling_factors(dim, total_bits - 1);
  }
  return config;
}
