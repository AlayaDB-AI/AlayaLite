...
inline void get_arr_shape(const py::buffer_info& buffer, size_t& rows, size_t& cols) {
    if (buffer.ndim != 2 && buffer.ndim != 1) {
        throw py::value_error("Input data must be a 1D or 2D array, got ndim=" + std::to_string(buffer.ndim));
    }
    if (buffer.ndim == 2) {
        rows = buffer.shape[0];
        cols = buffer.shape[1];
    ...
    explicit Index(
        ...
        if (degree < 32 || degree % 32 != 0) {
            throw py::value_error(
                "Laser Index: degree_bound must be a positive multiple of 32, got " + std::to_string(degree));
        }
        if (index_type != "QG") {
...
inline void register_laser_module(py::module_& m) {
    m.doc() = R"pbdoc(Laser on-disk Quantized Graph index (ported from symqg into alaya::laser).)pbdoc";
