...
class SearchBuffer {
 private:
  std::vector<AnnCandidate<T>, AlignedAllocator<AnnCandidate<T>>> data_;
 ...
 public:
  ...
  void resize(size_t new_size) {
    ...
    data_ = std::vector<AnnCandidate<T>, AlignedAllocator<AnnCandidate<T>>>(capacity_ + 1);
  }
  ...
  auto data() -> const std::vector<AnnCandidate<T>, AlignedAllocator<AnnCandidate<T>>> & {
    return data_;
  }
};
}  // namespace alaya