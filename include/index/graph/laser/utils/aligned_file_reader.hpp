...
inline void execute_io(io_context_t ctx, int fd, std::vector<AlignedRead> &read_reqs,
        ...
        while (n_tries <= n_retries) {
            ...
            if (ret != static_cast<int64_t>(n_ops)) {
                throw std::runtime_error(
                    "LinuxAlignedFileReader: io_submit() failed; returned " + std::to_string(ret)
                    + ", expected=" + std::to_string(n_ops)
                    + ", errno=" + std::to_string(errno) + "=" + ::strerror(static_cast<int>(-ret)));
            } else {
                ...
                if (ret != static_cast<int64_t>(n_ops)) {
                    throw std::runtime_error(
                        "LinuxAlignedFileReader: io_getevents() failed; returned " + std::to_string(ret)
                        + ", expected=" + std::to_string(n_ops)
                        + ", errno=" + std::to_string(errno) + "=" + ::strerror(static_cast<int>(-ret)));
                } else {
                    break;
...
inline int LinuxAlignedFileReader::submit_reqs(std::vector<AlignedRead> &read_reqs,
    ...
    if (read_reqs.size() > MAX_EVENTS) {
        throw std::runtime_error(
            "LinuxAlignedFileReader::submit_reqs: request count " + std::to_string(read_reqs.size())
    ...
    if (ret != static_cast<int>(n_ops)) {
        throw std::runtime_error(
            "LinuxAlignedFileReader::submit_reqs: io_submit() failed; returned " + std::to_string(ret)
            + ", expected=" + std::to_string(n_ops)
...
inline void LinuxAlignedFileReader::get_events(IOContext& ctx, int n_ops) {
    ...
    if (ret != static_cast<int64_t>(n_ops)) {
        throw std::runtime_error(
            "LinuxAlignedFileReader::get_events: io_getevents() failed; returned " + std::to_string(ret));
    }
}
