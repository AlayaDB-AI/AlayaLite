...
#include "utils/rbq_utils/fast_scan/highacc_fastscan.hpp"
...
class Lut {
 ...
 public:
  ...
  explicit Lut(const T *rotated_query, size_t padded_dim, bool use_hacc = false)
      : table_length_(padded_dim << 2), lut_(table_length_ * (static_cast<int>(use_hacc) + 1)) {
    // quantize float lut
    ...
    if (use_hacc) {
      ...
      scalar_quantize(lut_u16.data(), lut_float.data(), table_length_, vl_lut, delta_);
