...
class QuantizedGraph {
   ...
   private:
    ...
    [[nodiscard]] std::string gen_index_path(const char* prefix) const {
        return std::string(prefix) + "_R" + std::to_string(degree_bound_) + "_MD" + std::to_string(dimension_) + ".index";
    }

    void update_qg_out_of_memory(PID, const std::vector<Candidate<float>>&, LinuxAlignedFileReader& , ThreadData);
   ...
   public:
    ...
    void load_cache(std::string &cache_ids_file, std::string &cache_nodes_file, size_t online_cache_num);
    ...
    void batch_search(
        const float* __restrict__ query, uint32_t knn, uint32_t* __restrict__ results, size_t num_queries
    );
    ...
    void destroy_thread_data() {
        while(thread_data_.size() > 0) {
            ...
            if(data.pca_query_scratch_ != nullptr) {
                delete[] data.pca_query_scratch_;
            }
        }
...
inline QuantizedGraph::QuantizedGraph(size_t num, size_t max_deg, size_t main_dim, size_t dim)
    ...
    if(dimension_ != padded_dim_) {  // Currently only supports dimension that is a power of 2 (dimension_ must equal padded_dim_)
    ...
    std::cout << "main_dim: " << main_dim << ", dim: " << dim << ", dimension_: " << dimension_ << ", residual_dimension_: " << residual_dimension_ << std::endl;
    initialize();
}
...
#pragma omp critical
        {
            ...
            data.visited_ = HashBasedBooleanSet(std::min(this->num_points_ / 10, ef_search_ * ef_search_));
            data.sector_scratch_ = reinterpret_cast<char *>(memory::align_allocate<kSectorLen>(2 * max_beam_width_ * page_size_));
            data.pca_query_scratch_ = new float[dimension_ + residual_dimension_];  // Allocate PCA query buffer
 ...
 inline void QuantizedGraph::disk_search_qg(
    ...
    if(!medoids_.empty()) {
        ...
        for (size_t cur_m = 0; cur_m < medoids_.size(); cur_m++) {
            float cur_expanded_dist = space::l2_sqr(transformed_query, medoids_vector_.data() + (dimension_ + residual_dimension_) * cur_m, dimension_);
    ...
    auto process_node = [&](PID cur_node, float *cur_data) {
        ...
        if(residual_dimension_ > 0) {
            ...
            sqr_y += space::l2_sqr(reinterpret_cast<const float*>(residual_data), residual_query, residual_dimension_);
        }
    ...
    while (data.search_pool_.has_next()) {
        ...
        cur_beam_size = std::min(max_beam_width_, static_cast<size_t>(std::ceil(2 * static_cast<float>(cur_beam_size))));
        ...
        if (!frontier_read_reqs.empty()) {
            ...
            submit_time += std::chrono::duration_cast<std::chrono::microseconds>(submit_end - submit_start).count();
        }
    ...
    auto latency = std::chrono::duration_cast<std::chrono::microseconds>(query_end - query_start).count();
...
inline void QuantizedGraph::update_qg_out_of_memory(
    PID cur_id, const std::vector<Candidate<float>>& new_neighbors, LinuxAlignedFileReader& vector_reader, ThreadData thread_data
) {
    ...
    size_t full_page_size = ((dimension_ + residual_dimension_) * sizeof(float) + kSectorLen - 1) / kSectorLen * kSectorLen;
    ...
    for (size_t i = 0; i < cur_degree; ++i) {
        auto neighbor_id = new_neighbors[i].id;
        const auto* cur_data = reinterpret_cast<const float*>(reinterpret_cast<char*>(vector_buf) + i * full_page_size);
    ...
    rabitq_codes(
        x_rotated, c_rotated, packed_code_ptr, triple_x, factor_dq, factor_vq
    );
    ...
    for (size_t i = 0; i < cur_degree; ++i) {
        const auto* neighbor_data = reinterpret_cast<const float*>(
            reinterpret_cast<char*>(vector_buf) + i * full_page_size
        );
...
inline void QuantizedGraph::load_disk_index(const char* filename, float search_DRAM_budget) {
    ...
    if(!std::filesystem::exists(index_file_name_)) {
        throw std::runtime_error("QuantizedGraph::load_disk_index: file not found: " + index_file_name_);
    }
    ...
    size_t online_cache_num = std::min(cache_space / node_len_, static_cast<size_t>(kCacheRatio * num_points_));
...
inline void QuantizedGraph::load_medoids(const char *filename) {
    ...
    medoids_.resize(static_cast<unsigned long>(medoid_num) * static_cast<unsigned long>(tmp));
    medoid_input.read(reinterpret_cast<char*>(medoids_.data()), static_cast<std::streamsize>(sizeof(int) * medoid_num * tmp));
    medoid_input.close();
    ...
    if(medoid_num != static_cast<int>(medoids_.size())) {
        throw std::runtime_error("QuantizedGraph::load_medoids: medoid count mismatch between indices and vectors file");
    }
    if(dim != static_cast<int>(dimension_ + residual_dimension_)) {
        throw std::runtime_error("QuantizedGraph::load_medoids: medoid dimension mismatch vs. index dimension");
    }
    medoids_vector_.resize(static_cast<unsigned long>(medoid_num * (dimension_ + residual_dimension_)));
    mediod_vector_input.read(reinterpret_cast<char*>(medoids_vector_.data()), static_cast<std::streamsize>(sizeof(float) * medoid_num * (dimension_ + residual_dimension_)));
    mediod_vector_input.close();
}

inline void QuantizedGraph::load_cache(std::string &cache_ids_file, std::string &cache_nodes_file, size_t online_cache_num) {
    ...
    cache_ids_input.read(reinterpret_cast<char*>(cache_ids_.data()), static_cast<std::streamsize>(sizeof(PID) * online_cache_num));
    assert(tmp_node_len == node_len_);
    cache_nodes_.resize(online_cache_num * node_len_);
    cache_vectors_input.read(reinterpret_cast<char*>(cache_nodes_.data()), static_cast<std::streamsize>(sizeof(char) * online_cache_num * node_len_));
