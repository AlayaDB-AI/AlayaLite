...
class QGBuilder {
   ...
   public:
    explicit QGBuilder(
        QuantizedGraph& index, uint32_t ef_build, size_t num_threads
    )
        : qg_{index}
        , ef_build_{ef_build}
    ...
    #pragma omp critical
            {
                ...
                data.cur_page_scratch_ = reinterpret_cast<char *>(memory::align_allocate<kSectorLen>(qg_.page_size_));
                // Scratch for reading neighbor vectors (degree * vector_size)
                data.neighbor_vector_scratch_ = reinterpret_cast<char *>(memory::align_allocate<kSectorLen>(qg_.degree_bound_  * vector_tmp_page_size));
        ...
        // Each iteration processes ONE node: reads its neighbors, computes quantization, writes to disk.
...
#pragma omp critical
        ...
        cache_ids_output.write(reinterpret_cast<const char*>(qg_.cache_ids_.data()), static_cast<std::streamsize>(sizeof(PID) * cache_num));
        ...
        char *cache_buffer = reinterpret_cast<char *>(memory::align_allocate<kSectorLen>(batch_size  * qg_.page_size_));
        ...
        for(size_t i = 0; i < cache_num; i+= batch_size) {
            ...
            for (size_t j = 0; j < cur_batch_size; ++j) {
                frontier_read_reqs.emplace_back(
                    kSectorLen + (qg_.cache_ids_[i + j] * qg_.page_size_),
                    qg_.page_size_,
                    qg_.cache_ids_[i + j],
            ...
            for (size_t j = 0; j < cur_batch_size; ++j) {
                cache_vectors_output.write(reinterpret_cast<const char*>(cache_buffer + (j * qg_.page_size_)), static_cast<std::streamsize>(sizeof(char) * qg_.node_len_));
            }
        }
...
inline void QGBuilder::init_from_vamana(const std::string &filename) {
    ...
    uint32_t start;
    long file_offset = 0; // will need this for single file format support
    ...
    while (bytes_read != expected_file_size)
    {
        ...
        tmp.reserve(k);
        in.read(reinterpret_cast<char *>(tmp.data()), static_cast<std::streamsize>(k * sizeof(uint32_t)));

        for (PID cur_neigh : tmp) {
            in_degrees[cur_neigh]++;
            new_neighbors_[nodes_read - 1].emplace_back(cur_neigh, 0.0);  // Distance will be computed later in parallel during graph construction
        }
    ...
    // Sort node IDs by in-degree in descending order (high in-degree nodes are cached first for better search performance)
