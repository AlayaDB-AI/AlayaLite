...
/**
  * @brief Benchmark for L2 SQ8 SIMD distance functions
  *
  * Usage: ./l2_sqr_sq8_benchmark [dim1 dim2 ...]
...
void fill_random_uint8(std::vector<uint8_t>& v, unsigned seed) {
...
auto run_benchmarks_for_dim(size_t dim) -> DimResults {
  ...
  fill_random_uint8(y, 123);
  fill_min_max(min_vals, max_vals, dim, 456);

  // Generic (baseline)
  results.generic_.ns_per_call_ =
      run_benchmark(alaya::simd::l2_sqr_sq8_generic, x.data(), y.data(),
  ...
  if (features.avx2_ && features.fma_) {
    ...
    results.avx2_.ns_per_call_ =
        run_benchmark(alaya::simd::l2_sqr_sq8_avx2, x.data(), y.data(),
                      min_vals.data(), max_vals.data(), dim, kBenchmarkIterations);
  ...
  if (features.avx512f_) {
    ...
    results.avx512_.ns_per_call_ =
        run_benchmark(alaya::simd::l2_sqr_sq8_avx512, x.data(), y.data(),
                      min_vals.data(), max_vals.data(), dim, kBenchmarkIterations);
    results.avx512_.speedup_ = baseline_ns / results.avx512_.ns_per_call_;
  }
#endif

  // Best (get_l2_sqr_sq8_func with auto dispatch)
  results.best_.ns_per_call_ =
      run_benchmark(alaya::simd::get_l2_sqr_sq8_func(), x.data(), y.data(),
                    min_vals.data(), max_vals.data(), dim, kBenchmarkIterations);
...
void print_comparison_table(const std::vector<DimResults>& all_results) {
  ...
  for (const auto& r : all_results) {
    ...
    // Best (get_l2_sqr_sq8_func with dispatch)
  ...
  std::cout << "- **AUTO** = get_l2_sqr_sq8_func() with auto dispatch\n";
...
auto main(int argc, char* argv[]) -> int {
  std::cout << "# SQ8 L2 SIMD Distance Benchmark\n\n";
