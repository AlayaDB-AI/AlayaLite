...
class Lut {
 ...
 public:
  ...
  explicit Lut(const T *rotated_query, size_t padded_dim)
    ...
    // Here, 'the inner product (float) of every 4 dimensions from <x_u, P^(-1)·qr>' is quantized into a
    // knumBits-bit integer m. And it can be recovered to the nearest quantization boundary using:
    // vl_lut + m * delta
    ...
    // for quick calculation for <x_u,P^(-1)·qr>, get val_vec via LUT lookup and return :
    // sum_vl_lut_ + sum(val_vec) * delta_
  }
