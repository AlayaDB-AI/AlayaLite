 ...
 public:
  GraphPartitioner(const char *indexName, uint64_t num_nodes, size_t dim, uint64_t max_node_len, uint64_t node_per_page, unsigned cut, const char *data_type = "float",
    ...
    if (actual_size != expected_size) {
      throw std::runtime_error("Partitioner: index file size mismatch vs. expected header metadata");
    }
...
#pragma omp parallel for shared(reverse_graph_, full_graph_)
      ...
      throw std::runtime_error(std::string("Partitioner: system_error while loading vamana graph: ") + e.what());
    }
  }
